...
- Claude keychain: service `Claude Code-credentials`, account = macOS username for the default config dir. A custom `CLAUDE_CONFIG_DIR` gets its OWN item, and the key is the SERVICE, not the account: service `Claude Code-credentials-<first 8 lower-case hex of sha256(<absolute config dir, no trailing slash>)>`, account still the macOS username. Verified live 2026-09-15: `<MS_HOME>/claude/tulp` → `Claude Code-credentials-4f3610a9`, account `aadarwal`. No `.credentials.json` is written in the dir; a `.claude.json` with an `oauthAccount` object is. A scoped service can therefore never be the operator's own login — nothing needs attributing, and the unscoped service is never queried.
