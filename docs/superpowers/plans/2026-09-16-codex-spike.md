...
## Addendum 2 — the structured quota signal (from the Codex CLI's own source reading, 0.153.4)

A usage-limit turn IS persisted into the session's rollout file (the hook payloads' `transcript_path`) as a `task_complete` record: `{"timestamp":"…","type":"event_msg","payload":{"type":"task_complete","turn_id":"…","last_agent_message":null,"error":{"message":"…","codex_error_info":"usage_limit_exceeded"}}}` (snake_case; a successful turn writes the same record without the error). Standalone error events are not persisted; `notify` skips the quota branch (only `agent-turn-complete`, no error field); `tui.notifications` cannot run commands; there is no auto-exit-on-quota setting.

### Ruling (supersedes the per-turn watchdog above)
The trigger is the rollout record, read by ONE fleet watchdog per tmux server: `UserPromptSubmit` arms `ms _codex_watch` (45 s, `run-shell -b -d`) only if none is armed (compare-and-set under `withLock("codex-watch")`); the watchdog stats every in-flight Codex session's rollout, reads only the new bytes from a stored offset, and on a `task_complete` for the current turn with `codex_error_info: "usage_limit_exceeded"` appends `rate_limited` (and, when `MS_CODEX_AUTOROTATE=1`, opens a recovery and dispatches `_recover`); a `task_complete` without error becomes `stop`. It re-arms only while some Codex turn is in flight. No screen scraping in the trigger path; the cost is one short process per 45 s for the whole fleet, and none while Codex is idle.
