...
- **Superseded by the spike record's ruling (2026-09-16-codex-spike.md, Addendum):** a usage-limit turn fires NO Stop hook, so the wall signal is the absence of Stop, corroborated by the usage API; the installer writes TOML hook tables per account home and pre-trusts them with the verified hash recipe.
